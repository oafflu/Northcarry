...
export default function TermsManagementPage() {
  ...
  const loadTerms = async () => {
    setLoading(true)
    try {
      const result = await getCMSContent('terms')
      if (result.data) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const result = await saveCMSContent('terms', content)
      if (result.success) {
        toast.success('Terms of service saved successfully')
      } else {
        toast.error(result.error || 'Failed to save terms of service')
      }
    } catch (error) {
    ...
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage terms of service page content</p>
        </div>
      </div>
