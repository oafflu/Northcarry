...
import { createPostPurchaseUpsell, getAllCampaigns } from '@/app/actions/upsells'
import { getActiveProductsForAdmin, getProductVariantsForAdmin } from '@/app/actions/products'
import { ImagePicker } from '@/components/admin/image-picker'

export default function NewPostPurchaseUpsellPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const result = await createPostPurchaseUpsell({
        ...
        display_delay: parseInt(displayDelay) || 0,
        display_duration: displayDuration ? parseInt(displayDuration) : undefined,
      ...
      if (result.success) {
        toast.success('Post-purchase upsell created successfully')
        router.push('/admin/promos-upsells/post-purchase')
      } else {
        toast.error(result.error || 'Failed to create upsell')
      }
    } catch (error: any) {
      toast.error(error.message || 'Failed to create upsell')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Create Post-Purchase Upsell</h1>
