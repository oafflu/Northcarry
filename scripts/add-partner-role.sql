...
-- This script updates the role constraint to include the new partner role

-- Drop existing role constraints if they exist
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_role_check;
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS check_role;

-- Add the updated constraint that includes all roles including partner
ALTER TABLE profiles ADD CONSTRAINT check_role CHECK (role IN ('customer', 'admin', 'supplier', 'marketer', 'support', 'partner'));
